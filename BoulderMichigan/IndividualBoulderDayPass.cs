...
{
    public partial class frm_IndividualBoulderDayPass : Form
    {
        public frm_IndividualBoulderDayPass()
        {
            InitializeComponent();

        }

        public void radioButton_HalfDay_CheckedChanged(object sender, EventArgs e)
        {
            string halfDay = ("$20.00");
            textBox_Total.Text = halfDay;
        }

        private void radioButton_FullDay_CheckedChanged(object sender, EventArgs e)
        {
            string fullDay = ("$35.00");
    ...
    dateTimePicker_FitnessLabDayPass.Text == string.Empty || textBox_Phone.Text == string.Empty || textBox_Email.Text == string.Empty || (!radioButton_HalfDay.Checked && !radioButton_FullDay.Checked))
            ...
            else
            {
                ...
                {
                    string message = ($"Thank you for booking a visit with us, {textBox_FirstName.Text}. See you soon!");
                    MessageBox.Show(message);
                    this.Close();
                }
            }
